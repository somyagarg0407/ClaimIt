...
function EligibilityDashboard() {
  return (
    <motion.div
      ...
      className="relative w-full max-w-[480px] overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-lift"
    >
      {/* Browser chrome */}
      <div className="flex items-center gap-3 border-b border-gray-100 bg-gray-50/70 px-5 py-3.5">
        ...
        <div className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs text-gray-400">
      ...
      <div className="flex flex-col gap-5 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex items-center gap-4 rounded-2xl border border-gray-100 p-4">
            ...
            <div className="flex flex-col">
              <span className="text-xs font-medium text-gray-400">Eligibility score</span>
              <span className="text-sm font-semibold text-ink">Strong match</span>
            </div>
          </div>

          <div className="flex flex-col justify-between gap-3 rounded-2xl border border-gray-100 p-4">
            <div className="flex items-center gap-2 text-gray-400">
              <ListChecks className="h-4 w-4" />
              <span className="text-xs font-medium">Matched schemes</span>
            </div>
            <span className="tabular-mono text-2xl font-semibold text-ink">18</span>
          </div>
        </div>

        {/* Estimated benefits banner */}
        ...
        <div className="flex flex-col gap-1">
          ...
          {schemes.map((scheme) => (
            <div
              key={scheme.name}
              className="flex items-center justify-between rounded-xl px-2 py-3 transition-colors hover:bg-brand-25"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-gray-400">{scheme.category}</span>
                </div>
              </div>
              <span className="tabular-mono text-xs font-semibold text-brand-600">{scheme.match}% match</span>
            </div>
          ))}
        </div>
      </div>
